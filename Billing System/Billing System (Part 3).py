...
class Billing_App:
    def __init__(self,root):
        ...
        """Button"""
        bill_btn = Button(F1,text="Search",width=10,bd=7,font=("arial",12,"bold"),command=self.find_bill).grid(row=0,column=6,padx=10,pady=10)
        ...
        self.txtarea=Text(F5,yscrollcommand=scrol_y.set,state=DISABLED)
        ...
        Clear_btn=Button(btn_f,text="Clear",bg="cadetblue",fg="white",pady=15,width=11,font=("arial",13,"bold"),bd=5,command=self.clear_data).grid(row=0,column=2,pady=5,padx=5)
        Exit_btn=Button(btn_f,text="Exit",bg="cadetblue",fg="white",pady=15,width=11,font=("arial",13,"bold"),bd=5,command=self.Exit_App).grid(row=0,column=3,pady=5,padx=5)
    ...
    def save_bill(self):
        ...
        if op>0:
            ...
            file=open("Customer Bills/"+str(self.bill_no.get())+".txt","w")
            file.write(self.bill_data)
            file.close()
            messagebox.showinfo("Sucess","Bill Saved Succesfully")
    ...
    def find_bill(self):
        ...
        for i in os.listdir("Customer Bills/"):
            if i.split('.')[0]==self.search_bill.get():
                file=open(f"Customer Bills/{i}","r")
                self.txtarea.delete('1.0',END)
                for d in file:
                    self.txtarea.config(state=NORMAL)
    ...
    def clear_data(self):
        ...
        self.bill_no.set(x)
        self.txtarea.delete('1.0',END)
        self.txtarea.insert(END,self.welcome_bill)
   
    def Exit_App(self):
        op=messagebox.askyesno("Exit App","Do you want to exit")
        if op>0:
            self.root.destroy()
