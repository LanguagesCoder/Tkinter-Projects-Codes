...
class ReportClass:
    def __init__(self,root):
        ...
        #=====Clear=====
        btn_clear=Button(self.root,text="Clear",font=("goudy old style",15,"bold"),bg='gray',fg="white",cursor="hand2",command=self.clear).place(x=790,y=100,width=100,height=35)
        #=====Labels=====
    ...
    def search(self):
        ...
        try:
            if self.var_search.get()=="":
                messagebox.showerror("Error","Roll Number Is Requires",parent=self.root)
            else:
...
#=================================================================================================
    def delete(self):
        ...
        try:
            if self.var_id=="":
                messagebox.showerror("Error","Please Search The Student",parent=self.root)
                ...
                if row==None:
                    ...
                    op=messagebox.askyesno("Confirm","Are You Sure You Wnat To Delete This Result...")
                    if op == True:
                        cur.execute('delete from result where rid=?',(self.var_id,))
                        con.commit()
                        messagebox.showinfo("Deleted","Result has been deleted...",parent=self.root)
                        self.clear()
        except Exception as ex:
            messagebox.showerror("Error",f"Error Due To : {str(ex)}",parent=self.root)
#=================================================================================================
